...
window.onload = () =>{
    var preview = $('<div  id  = "preview" position = "absolute" ></div>');
$.each(images, function(index,jsonObject){
        $(".gallery").append('<img src= "images/square/' + jsonObject.path + '" alt = "' + jsonObject.title +'"> ');  
});
...
$(".gallery").children()
    .mouseenter(function( event ) {
       ...
       for (var j = 0; j < images.length; j++) {
           if (images[j].title == $(this).attr("alt")) {
               currentImg = images[j];
           }
       }
       
       var captionstring = '<p> '
       + currentImg.title + '<br> ' + currentImg.city + ', ' + currentImg.country + ' [' + currentImg.taken + '] </p>';
			
			preview.html('<img src="images/medium/' + currentImg.path + '" alt="' + currentImg.title + '"/> ' + captionstring);
    ...
    .mousemove(function( event ) {
        preview.css('left', event.pageX + 10);
